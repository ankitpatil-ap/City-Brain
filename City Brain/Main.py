...
class ads:
    def __init__(self,root):
        self.root=root
        self.root.title("City Brain.")
        self.root.geometry("1280x780+20+20")
        self.bg=ImageTk.PhotoImage(file="mbg.jpg")
        bg=Label(self.root,image=self.bg).place(x=0,y=0,relwidth=1,relheight=1)
        title=Label(root,text="City Brain", font=("times new roman",30,"bold"),bg="white",fg="Blue").place(x=650,y=30)
...
# Get the coordinates of faces
    ...
    def send_mail_function(self):
            ...
            try:
                ...
                server.ehlo()
                server.starttls()
                server.login("abc@gmail.com",'abc@2021')
...
# Enter file name for example "ak47.mp4" or press "Enter" to start webcam
    def value(self):
            ...
            output_layers = [layer_names[i[0] - 1] for i in net.getUnconnectedOutLayers()]
            colors = np.random.uniform(0, 255, size=(len(classes), 3))
            val = input("Press enter to start webcam or Insert Video File Name: \n")
            if val == "":
                    val = 0
            return val
